...
void OLED_DISPLAY_CLEAR_OneLine(u8 l){//只清屏一行
	u8 j,t,r;
	r = (0xB0 + l) + 2;
	for(t=0xB0 + l; t<r; t++){	//设置起始页地址为0xB0
		I2C_SAND_BYTE(OLED0561_ADD,COM,t); 	//页地址（从0xB0到0xB7）
		I2C_SAND_BYTE(OLED0561_ADD,COM,0x10); //起始列地址的高4位
		I2C_SAND_BYTE(OLED0561_ADD,COM,0x00);	//起始列地址的低4位
		for(j=0;j<132;j++){	//整页内容填充
...
//取反显示英文与数字8*16的ASCII码
//取模大小为8*16，取模方式为“从左到右从上到下”“纵向8点下高位”
void ANTI_OLED_DISPLAY_8x16(u8 x, //取反ANTI显示英文数字 显示汉字的页坐标（从0到7）（此处不可修改）
						u8 y, //显示汉字的列坐标（从0到128）
						u16 w){ //要显示汉字的编号
	u8 j,t,c=0;
	y=y+2; //因OLED屏的内置驱动芯片是从0x02列作为屏上最左一列，所以要加上偏移量
...
//显示反码的字符串,向LCM发送一个字符串,长度64字符之内。
//应用：OLED_DISPLAY_8_16_BUFFER(0," DoYoung Studio"); 
void ANTI_OLED_DISPLAY_8x16_BUFFER(u8 row,u8 *str){
	u8 r=0;
	while(*str != '\0'){
		ANTI_OLED_DISPLAY_8x16(row,r*8,*str++);
		r++;
    }	
}
...
void OLED_DISPLAY_16x16(u8 x, //显示汉字的页坐标（从0xB0到0xB7）
	...
	for(t=0;t<2;t++){
		...
		I2C_SAND_BYTE(OLED0561_ADD,COM,y%16);	//起始列地址的低4位
		for(j=0;j<16;j++){ //整页内容填充
 			I2C_SAND_BYTE(OLED0561_ADD,DAT,*china++);
			}x++; //页地址加1
	}
	I2C_SAND_BYTE(OLED0561_ADD,COM,0xAF); //开显示 
}
...
//取反显示汉字16*16
//取模大小为16*16，取模方式为“从左到右从上到下”“纵向8点下高位”
void ANTI_OLED_DISPLAY_16x16(u8 x, //取反显示 ,显示汉字的页坐标（从0xB0到0xB7） 
			u8 y, //显示汉字的列坐标（从0到63）
			uc8 *china){ //要显示汉字的编号
	u8 j,t;
	y=y+2; //因OLED屏的内置驱动芯片是从0x02列作为屏上最左一列，所以要加上偏移量
...
//显示一串汉字16*16
//取模大小为16*16，取模方式为“从左到右从上到下”“纵向8点下高位”
void OLED_DISPLAY_Buff_16x16(u8 x, //显示汉字的页坐标（从0xB0到0xB7）
			u8 y, //显示汉字的列坐标（从0到127）
			uc8 *china,  //要显示汉字的编号
			u8 num){ //要显示的汉字个数,与数组对应
	u8 j,t,n;
	y=y+2; //因OLED屏的内置驱动芯片是从0x02列作为屏上最左一列，所以要加上偏移量
	for(n=0; n<num; n++){
		if(n>0) y = y + 16;//第一次不加16
		for(t=0;t<2;t++){
			I2C_SAND_BYTE(OLED0561_ADD,COM,0xb0+x); //页地址（从0xB0到0xB7）
			I2C_SAND_BYTE(OLED0561_ADD,COM,y/16+0x10); //起始列地址的高4位
			I2C_SAND_BYTE(OLED0561_ADD,COM,y%16);	//起始列地址的低4位
...
//反白背景显示一串汉字16*16
//取模大小为16*16，取模方式为“从左到右从上到下”“纵向8点下高位”
void ANTI_OLED_DISPLAY_Buff_16x16(u8 x, //显示汉字的页坐标（从0xB0到0xB7）
			u8 y, //显示汉字的列坐标（从0到127）
			uc8 *china,  //要显示汉字的编号
			u8 num){ //要显示的汉字个数,与数组对应
	u8 j,t,n;
	...
	for(n=0; n<num; n++){
		...
		for(t=0;t<2;t++){
			...
			I2C_SAND_BYTE(OLED0561_ADD,COM,y%16);	//起始列地址的低4位
			for(j=0;j<16;j++){ //整页内容填充
	 			I2C_SAND_BYTE(OLED0561_ADD,DAT,~*china++); //取反反白背景显示
				}x++; //页地址加1
		}
		x-=2; //把行值恢复,用于下个汉字
		I2C_SAND_BYTE(OLED0561_ADD,COM,0xAF); //开显示
